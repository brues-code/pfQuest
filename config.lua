...
pfQuestConfig.close:SetSize(20, 20)
...
pfQuestConfig.welcome:SetSize(160, 28)
...
pfQuestConfig.save:SetSize(160, 28)
  ...
  for _, data in pairs(config) do
    if data.type then
      ...
      elseif data.type == "slider" then
        ...
        frame.input.thumb:SetSize(8, 14)
      ...
      if frame.input and pfUI.api.emulated and data.type ~= "slider" then
        local inputW, inputH = frame.input:GetSize()
        frame.input:SetSize(inputW / 0.6, inputH / 0.6)
...
do -- welcome/init popup dialog
  ...
  for i, button in pairs(buttons) do
    ...
    pfQuestInit[i].bg:SetSize(200, 200)
  ...
  pfQuestInit.checkbox:SetSize(22, 22)
  ...
  pfQuestInit.save:SetSize(100, 24)
