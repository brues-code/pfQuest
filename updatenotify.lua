local version = GetAddOnMetadata("pfQuest", "Version")
if version == "@project-version@" then return end

local strsplit = strsplit or function(delimiter, subject)
  if not subject then
    return nil
  end
...
local ADDON_PREFIX = "pfQuest-CAPI"
...
versioncheck:SetScript("OnEvent", function()
  if event == "ADDON_LOADED" then
    if arg1 == "pfQuest" then
      major, minor, fix = strsplit(".", tostring(version))
  ...
  for _, chan in pairs(channels) do
    SendAddonMessage(ADDON_PREFIX, "VERSION:" .. version, chan)
  end
